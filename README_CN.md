...
面向真实 Linux 节点切换时，`deploy.sh` 现在也支持一条命令接管现有的用户态 Reticulum 节点：

```bash
bash deploy.sh --adopt-existing-reticulum vicliu
```

这个模式会自动导入该用户已有的 `.reticulum/config`，联动 `lxmd`，mask 旧的独立 `rnsd.service`，并安装更顺手的 `reticulum.service` / `lxmf.service` 别名。更完整的说明见 `DEPLOYMENT.md`。
